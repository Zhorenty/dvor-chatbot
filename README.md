...
- `/my_bookings` (или кнопка `Мои записи` внутри профиля) — открывает список записей (актуальные/прошедшие)
...
- Личка (админский сценарий):
  - кнопка `Обновить Google Docs` — принудительная синхронизация данных из Google Docs (расписание + тренеры) и ссылка на документ
  ...
  - кнопка `Управление записями`:
    - `Список записей` -> выбор сегмента (`Активные`/`Архивные`) -> выбор категории -> выбор записи по страницам (кнопки «Предыдущая/Следующая страница»)
  ...
  - `/approve_payment <id>`, `/approve_partial_payment <id>` и `/reject_payment <id>` — модерация подтверждений оплаты
  ...
  - при заданном `ADMIN_CHAT_ID` автоматически отправляет экономическую сводку:
    - по завершенной неделе (раз в новую неделю),
    - по завершенному месяцу (раз в новый месяц),
    - со структурой выручки: итоги, категории, топ мероприятий
  - ручной запрос экономической сводки:
    - кнопка `Экономическая сводка` (выбор периода в UI)
    - команда `/economic_summary <period>`
...
3. Нажмите `Записаться`, затем `Профиль`, после оплаты отправьте файл с подтверждением (документ/фото).
4. Если ваш user id указан в `ADMIN_USER_IDS`, нажмите `Обновить Google Docs` и проверьте `/payments_queue`.
