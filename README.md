...
- Личка (админский сценарий), главное меню:
  ...
  - `Абонементы` — сразу фильтры (`Активные` / `Скоро истекают` / `На проверке` / `Отмененные`) и `Поиск`; `На проверке` — очередь модерации
  ...
  - `Инструменты`:
    ...
    - `Список записавшихся`, `Список дворян`, `Поиск по пользователю`
...
5. Если ваш user id указан в `ADMIN_USER_IDS`, откройте `Инструменты` → `Обновить Google Sheets` и проверьте `Заявки на оплату` и `Абонементы`.
