...
- Личка (пользовательский сценарий):
  ...
  - `/trainings` или кнопка `Расписание` — показывает ближайшие тренировки, походы и трейлы
...
- Личка (админский сценарий):
  ...
  - `/payments_queue` — очередь оплат на проверку (каждая заявка приходит отдельным сообщением)
