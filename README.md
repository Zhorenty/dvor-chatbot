...
- Личка (пользовательский сценарий):
  ...
  - `/trainings` или кнопка `Расписание` — показывает ближайшие тренировки
  - `/book` или кнопка `Записаться` — записывает на ближайшую тренировку
  - после записи отправляет реквизиты/инструкцию оплаты
  - `/my_bookings` или кнопка `Мои записи` — показывает все записи и их статусы
...
- Личка (админский сценарий):
  - кнопка `Обновить расписание` — принудительная синхронизация расписания
  - кнопка `Список записавшихся` — показывает по каждой ближайшей тренировке список участников с тегами
  - `/payments_queue` — очередь оплат на проверку
...
- Групповой сценарий:
  - при входе нового участника бот пытается отправить приветствие в личку
  - если личка недоступна, отправляет fallback-сообщение в группу (опционально)
